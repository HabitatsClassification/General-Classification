...
algo <- c(# NeuralNetwork
  ...
  "pcaNNet", "rbfDDA", 
  # Random Forest
  "cforest", "extraTrees", "ranger", "Rborist",
  "rf", "rFerns", "RRF", "RRFglobal", "wsrf",                
  # Logistic Regression
  "LMT", "LogitBoost", "polr", "vglmAdjCat",
  # SVM
...
multiclass <- function(n){
  ...
  for(j in 1:n){
    ...
    resultado[[j]] <- data.frame(row.names = names(ensemble),
                                 auc = preds_auc)
  }
  return(resultado)
}

result_multiclass4 <- multiclass(10)
df_multiclass <- do.call(rbind, lapply(result_multiclass,unlist))
df_multiclass <- cbind(result_multiclass3[[1]],result_multiclass3[[3]],
                       result_multiclass3[[4]],result_multiclass3[[6]],
                       result_multiclass3[[7]],result_multiclass3[[8]],
                       result_multiclass[[1]],result_multiclass2[[1]]
)
colnames(df_multiclass) <- algo
apply(df_multiclass, 1, mean)
apply(df_multiclass, 1, max)
...
ensemble <- caretList(reformulate(termlabels = colnames(train)[-1], response=colnames(train)[1]),
                      ...
                      tuneList = list(
                        naive_bayes = caretModelSpec(method = "nb", 
                                                     tuneGrid = expand.grid(fL = 0,
                                                                            usekernel = TRUE,
